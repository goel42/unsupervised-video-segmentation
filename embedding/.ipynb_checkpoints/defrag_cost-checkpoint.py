...
def defrag_cost(frames_batch, labels_batch, W):
    ...
    #dim should be latent_space_dim * BATCH_Size
    labels_latent_space = W_a * labels_batch.T #remember label_batch has word2vecs of label strings
    ...
    column_diffs = np.zeors(__batch_size, __batch_size)
    row_diffs = np.zeros(__batch_size, __batch_size)
    for i in range(__batch_size):
        curr_column_diff = max(0, dots[:, i] - dots[i,i] + __gmargin)
        curr_column_diff[i] = 0
        column_diffs[:,i] = curr_column_diff
        
        curr_row_diff = max(0,dots[i,:] - dots[i,i] + __gmargin)
        curr_row_diff[i] = 0
    ...
    #MAJOR #DOUBT why transpose of ltop in line 135
    frames_deltas = #TODO after clarifying
    labels_deltas = #TODO after clarifying
    
    #backprop frame mapping
    df_W_f = frames_deltas * frames_batch
    
    #backprop label mapping
    df_W_a = labels_deltas * labels_batch

    #TODO: verify iwth data 
    theta = np.concatenate([W_f.flatten(), W_a.flatten()])
    cost = {
        "raw_cost": gcost,
        "reg_cost": params.embedding.train.reg_const_gamma/2 * np.sum(theta**2), #DOUBT: why divide by 2?
        "total_cost": None
    }
    ...
    #TODO: verify iwth data 
    W_gradient = {
        "Wf_gradient": df_W_f + params.embedding.train.reg_const_gamma*W_f
        "Wa_gradient": df_W_a + params.embedding.train.reg_const_gamma*W_a
    }
